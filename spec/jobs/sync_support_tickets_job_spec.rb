...
RSpec.describe SyncSupportTicketsJob do
  describe "#perform" do
    it "does nothing when sync is disabled" do
      create(:sync_setting, key: "support_tickets", enabled: false)

      expect(SupportTicketsExtractor).not_to receive(:new)

      described_class.new.perform
    end

    it "calls extractor and marks completed when enabled" do
      setting = create(:sync_setting, key: "support_tickets", enabled: true, last_synced_at: 1.hour.ago)

      fake_client = instance_double(JiraClient)
      allow(JiraClient).to receive(:new).and_return(fake_client)

      extractor = instance_double(SupportTicketsExtractor, call: nil)
      expect(SupportTicketsExtractor).to receive(:new)
    ...
    it "marks failed and re-raises when extractor raises" do
      setting = create(:sync_setting, key: "support_tickets", enabled: true)

      fake_client = instance_double(JiraClient)
      allow(JiraClient).to receive(:new).and_return(fake_client)

      extractor = instance_double(SupportTicketsExtractor)
      allow(SupportTicketsExtractor).to receive(:new).and_return(extractor)
